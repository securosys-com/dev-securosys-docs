...
export default function DocsPluginIdDropdownNavbarItem(props) {
  ...
  const docsVersions = Object.fromEntries(
    useThemeConfig().navbar.items.filter(
      (item) => (item.type === 'docsVersionDropdown') && (item.x_dropdownlabel_api_reference !== undefined)
  ...
  dropdownProps.label = docsVersions[activeDocsPluginId]?.x_dropdownlabel_api_reference ?? dropdownProps.label;

  dropdownProps.items = Object.values(docsVersions).map(
    (item) => {
    return {
      "type":"docsVersion",
      "label": item.x_dropdownlabel_api_reference,
