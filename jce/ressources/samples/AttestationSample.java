 ...
 */
public class AttestationSample {
	...
	public static void main(final String... args) throws Exception {
		...
		PrimusAttestation.createRsaAttestationKey(attestationKeyName);

		// get the attestation key certificate chain (Securosys CA - device key - attestation key)
		final Certificate[] cc = PrimusAttestation.getRsaAttestationKeyCertificateChain(attestationKeyName);
		System.out.println();
		System.out.println("rsa attestation key certificate chain length is: " + cc.length);
		final Certificate ca = cc[cc.length - 1];
		final Certificate attestationCertificate = cc[0];

		// verifying version of above
		final Certificate[] cc2 = PrimusAttestation.getRsaAttestationKeyCertificateChain(attestationKeyName, ca);
		assert Arrays.deepEquals(cc, cc2);

		// 3 -- GET KEY ATTRIBUTES AND VERIFY THEM

		// generate a new key. In real application the key would be retrieved in need of attestation
		final String keyname = "rsa-key-" + UUID.randomUUID().toString();
		final Key key = ((KeyPair)PrimusName.generate(KeyPairGenerator.getInstance("RSA", PrimusProvider.getProviderName()), 2048, keyname)).getPrivate();
		...
		final String rsaSignAlgorithm = PrimusAttestation.getRsaSignAlgorithm();
		final Signature verifier = Signature.getInstance(PrimusPrimitives.getRsaSsaPssalgorithmFor(rsaSignAlgorithm));
		try {
			// if the signaure is RSA/PSS:
			verifier.setParameter(PrimusPrimitives.getPssParameterSpecFor(rsaSignAlgorithm));
		} catch (final IllegalArgumentException e) {
			// ignored, for the non-RSA/PSS case
		}
