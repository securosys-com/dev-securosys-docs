 ...
 * Using PrimusIes.getAsyncEncryptStream, which is new to 2.3.2.
 */
public class IesXmlAsyncStreamSample2 {
...
//		}
		...
		try (final FileInputStream sourceStream = new FileInputStream(input)) {
			...
			ies.setCurve(curve);
			ies.initKey(targetKey.getPublic());
			final InputStream encryptStream = ies.getAsyncEncryptStream(sourceStream);
...
//			}
			...
			try (final FileOutputStream targetStream = new FileOutputStream(decrypted)) {
				final PrimusIes ies2 = new PrimusIes(PrimusIes.Mode.XML);
				ies2.initKeys(targetKey);
				ies2.decryptStream(encryptStream, targetStream);
			}
		}
